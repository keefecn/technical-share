...
**J2EE****是**SUN公司基于Java的体系结构，是4层结构，包括客户层(客户端), WEB层(JSP/Servlets), 业务层(Java Bean), EIS层(DBMS)
...
**（1****）Java SE****动态代理：**     使用动态代理可以为一个或多个接口在运行期动态生成实现对象，生成的对象中实现接口的方法时可以添加增强代码，从而实现AOP。缺点是只能针对接口进行代理，另外由于动态代理是通过反射实现的，有时可能要考虑反射调用的开销。
