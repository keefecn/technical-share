...
![1574501244900](E:\project\technical-share\media\sf_reuse\algo_base_001.png)
...
​        ![1574501295788](E:\project\technical-share\media\sf_reuse\algo_base_002.png)                                          
...
l  外部排序指的是大文件的排序，即待排序的记录存储在[外存储器](https://baike.baidu.com/item/外存储器)上，待排序的文件无法一次装入内存，需要在内存和外部存储器之间进行多次数据交换，以达到排序整个文件的目的。外部排序最常用的算法是多路归并排序，即将原文件分解成多个能够一次性装入内存的部分分别把每一部分调入内存完成排序。然后，对已经排序的子文件进行归并排序。

l  内排序是可以在内存中完成的排序。一般提到排序都是指内排序，比如[快速排序](https://baike.baidu.com/item/快速排序)，[堆排序](https://baike.baidu.com/item/堆排序)，[归并排序](https://baike.baidu.com/item/归并排序)等。
...
/*基本思想是：任取待排序列的某个记录作为基准，按照该关键码大小，将整个序列分成两个序列——左侧的所有记录的关键码都比基准小（或者等），右侧的都比基准大，基准则放在两个子序列之间。分别对左右子序列重复上面的过程，直到最后所有的记录都放在相应的位置,充分体现了分冶法的策略：将大问题分解成可解决的小问题*/
...
step3：遍历前序，每次调用FINDLR判断结点是否左右，可建立唯一的二叉树，如下：
   ...
   ![1574501332140](E:\project\technical-share\media\sf_reuse\algo_base_003.png)
...
备注：常用**字符串哈希函数有****BKDRHash****，APHash****，DJBHash****，JSHash****，RSHash****，SDBMHash****，PJWHash****，ELFHash****等等**。
...
如：（M=3）
   ...
   ![1574501384878](E:\project\technical-share\media\sf_reuse\algo_base_004.png)
...
​       如：（M=3）
   ...
   ![1574501511163](E:\project\technical-share\media\sf_reuse\algo_base_005.png)
...
​       是B+树的变体，在B+树的非根和非叶子结点再增加指向兄弟的指针；
   ...
   ![1574501542793](E:\project\technical-share\media\sf_reuse\algo_base_006_tree.png)
...
下图画出了Bloom Filter 的工作原理:
   ...
   ![1574501572255](E:\project\technical-share\media\sf_reuse\algo_base_007.png)

l  Insert 时，输入A 给定之后，k 个互相独立的Hash Function 将它们映射到m 位中的k 个位上，并且将这位置1。如此，Bloom Filter 就记住了元素A 的“案底”，可备以后查询用。

l  Query 时，输入A 给定之后，我们一样得到k 个位。当它们都为1 时，就认为元素A 是之前Insert 的某个成员。
...
l  (BloomFilter位数组大小m)/(实际的元素个数n)越大，错误率越低，但消耗的空间会越多。

l  哈希表是K=1的bloom filter.  
...
l  插入排序：是插入到有序队列，与有序队列依个比较。

l  选择排序：每次选择一个次关健值加入到队列中。如heapSort.

l  交互排序：比较两值大小，进行交换，快速排序是交互排序中冒泡的改进，从待排队列中选取一值作为枢，然后与枢比较，将值分布在枢左右，最后对左右子序列进行递归排序。
...
l  [Cracking the Coding Interview：150 Programming Questions and Solutions](http://www.amazon.com/gp/product/098478280X/ref=as_li_tl?ie=UTF8&camp=1789&creative=390957&creativeASIN=098478280X&linkCode=as2&tag=htcatu-20&linkId=B6WXIEKJHEBBWJ7B)

l  [Introduction to Algorithms, 3rd Edition](http://www.amazon.com/gp/product/0262033844/ref=as_li_tl?ie=UTF8&camp=1789&creative=390957&creativeASIN=0262033844&linkCode=as2&tag=htcatu-20&linkId=J2PHCTWEAND3YQF4)

l  [Data Structures and Algorithms in Java (2nd Edition)](http://www.amazon.com/gp/product/0672324539/ref=as_li_tl?ie=UTF8&camp=1789&creative=390957&creativeASIN=0672324539&linkCode=as2&tag=htcatu-20&linkId=Y4KPAZH5PFHYXMBA)

l  [High Performance JavaScript (Build Faster Web Application Interfaces)](http://www.amazon.com/gp/product/059680279X/ref=as_li_tl?ie=UTF8&camp=1789&creative=390957&creativeASIN=059680279X&linkCode=as2&tag=htcatu-20&linkId=WUNMGD5EARDJZKTT)
