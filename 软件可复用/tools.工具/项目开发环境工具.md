...
step4:  打开脚本文件，vscode菜单 - 运行 - 启动调试。也可以将启动信息脚配置在 launch.json，参考如下，
...
```json
{
    ...
    // name有三个选项：Integrated Terminal，External Terminal即另起命令行，Attach
    // program 可更换要启动的文件
    "version": "0.2.0",
    "configurations": [    
        {
...
<img title="" src="file:///C:/mydocs/repos/technical-share/media/sf_reuse/tools/tools_vscode_001.png" alt="VSCode 插件架构" data-align="inline">
...
* Eclipse 跌落“神坛”，VS Code 等新兴 IDE 后来居上！  www.myzaker.com/article/61dea40e8e9f0940a715a5e6
...
备注：rpm--redhat package manager。dnf用来取代yum。
...
| [Go](http://en.wikipedia.org/wiki/Go!_(programming_language))                                          | v1.11推出 modules 机制，简称 mod。<br>法1：go get xx<br>法2：go install xx                    | import (                                                                                                             |
| "xx"                                                                                                   |                                                                                   |                                                                                                                      |
| )                                                                                                      |                                                                                   |                                                                                                                      |
...
默认情况下，pypiserver 的上传操作是 <u>密码保护</u>的。
...
* 《[rose建模视图指导](tools.工具/rose建模视图指导.md)》
* 《[EA用户手册](EA用户手册.md)》
...
* 隧道：本机隧道相当于创建一个新端口映射到本机的SSH端口，相当于一个SSH通道。如果目标机提供服务

**用途1**：本机登陆受限制的目标机。SSH通道，二层SSH连接，通过代理服务器+隧道技术，通过堡垒机登陆到目标机。要求先登陆堡垒机（让隧道临时建立），才能再开连接登陆目标机。

* 堡垒机：一台，不需要代理服务器。建立连接后，设置本机隧道（参数：SOCKET5, 监听端口如25000），如下图所示，
  
  ![image-20210918171413230](../../media/sf_reuse/tools/tools_finalshell_001.png)

* 目标机：多台，使用代理服务器（参数是堡垒机的隧道参数）。可以不用创建隧道。
  
  ![image-20210918171740047](../../media/sf_reuse/tools/tools_finalshell_002.png)
...
![img](../../media/sf_reuse/tools/tools_filezilla_001.png)
...
![img](../../media/sf_reuse/tools/tools_sourceinsight_001.png)
...
* nodejs教程  https://www.runoob.com/nodejs/nodejs-npm.html  
