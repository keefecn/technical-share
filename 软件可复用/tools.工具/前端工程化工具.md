...
| terser-webpack-plugin              | 压缩代码         | webpack-4.0默认                                              |
...
webpack-dev-server为了加快打包进程是将打包后的文件放到内存中的，所以我们在项目中是看不到它打包以后生成的文件／文件夹的，但我们仍然需要配置路径：

* output.path这个参数是配置 打包文件的保存路径 的，
