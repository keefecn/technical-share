...
* varnishadm  命令行终端，可以在终端上执行各种管理命令。
* varnishstat   统计varnish运行状态
* varnishlog    日志. 一个请求会产生二条日志记录。
* varnishtop
...
Apache Traffic Server（ATS或TS）软件是一种快速、可扩展和可扩展的 HTTP/1.1 和 HTTP/2 兼容缓存代理服务器。 以前的商业产品，雅虎！ 捐赠给了 Apache 基金会，目前被几个主要的 CDN 和内容所有者使用。
...
* 《[nginx用户手册](../tools.工具/nginx用户手册.md)》
