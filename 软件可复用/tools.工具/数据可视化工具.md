...
* 数据连接：指报表元数据连接。**内置数据库**是本地数据库HSQL，用户初学者可以使用这个。**外置数据库**一般用在正式场合，可以在初次连接时选择；也可以在登陆网页端后切换，菜单路径： ` 管理系统 - 系统管理 - 常规 - 常规参数 - 外接数据库`。FineBI也是类似配置。
...
# 启动. 如果是重启，则杀finebi相应进程，再启动。更新DB驱动包后，需重启服务。
