...
1) make mrproper //clean all generate file include .config

2) make menuconfig/xconfig/oldconfig

3) make 

4) make modules

5) make modules_install  //cp modules to /usr/lib/$version

6) make install  //cp vmlinuz, System.map to /boot

7) mkinitramfs -o [dest_dir] [version]  //generate the initrd.img
...
http://www.ibm.com/developerworks/cn/linux/l-lkm/?ca=drs-tp3208
...
**用户空间函数使用系统调用**：如glibc函数使用系统调用。通过自定义系统调用号NR_XXX传入内核，内核通过NR_XXX找到对应的内核实现函数，通常是sys_XXX. 自定义系统调用号需在内核sys_call_table中注册。Linux的系统调用通过int 80h实现，用系统调用号来区分入口函数。

**内核空间系统调用实现**：在unistd.h中定义系统调用号_NR_XX； /linux/kernel/sys.c源文件中写系统调用函数xx；在/linux/sys.h将函数名加放到sys_call_table[]；用户空间使用 `syscallx( ＿NR＿XX)`调用；
...
inline: 将函数代码合并到调用程序中，类似macro,但有类型参数检测。

static: 修饰变量时只在本文件/类/函数有效。
...
**[2]**  /proc文件系统全面观 http://blog.csdn.net/wqf363/archive/2006/10/31/1359189.aspx 