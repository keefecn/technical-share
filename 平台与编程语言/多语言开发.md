...
| asp                                                          |                                                              |
| jsp                                                          | <%--   注释 --%>                                             |
| html/   xm*                                                  | <!--这儿是注释-->,    '--'只能在注释的始末位置，不可置入注释文字区域。 |
| css                                                          | /* */                                                        |
| javascript                                                   | //单行                                                       |
|                                                              |                                                              |
...
| ADT                                          | [Android Development   Tools (ADT)](http://developer.android.com/sdk/eclipse-adt.html) | http://developer.android.com/sdk/eclipse-adt.htm*            |                             | 淘汰 |
...
| scala                                        | scala语言插件[Scala IDE(for Scala 2.9.x and Eclipse Juno)](http://download.scala-ide.org/sdk/e38/scala29/stable/update-site.zip) | http://www.scala-lang.org/download/2.7.6.final.htm*          |                             |      |
...
| [maven](http://maven.apache.org) | 构建工具。                                                   | pom.xm*                          | mvm instal*   mvm clean instal*   mvm package   | 常JAVA项目     |
...
*2.gradle**慢慢替代**maven**和**ant**。**maven**可以两个命令先后招待，如**clean**后**install**为**mvn clean install.*
 
表格 22 maven和ant的比较列表

|                  | Maven                                                        | Ant                                                          |
| ---------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 标准构建文件     | project.xml 和 maven.xm*   （后面版本二合一为pom.xml）       | build.xm*                                                    |
...
*备注：**Maven* *和* *Ant* *针对构建问题的两个不同方面。**Ant* *为* *Java* *技术开发项目提供跨平台构建任务。**Maven* *本身描述项目的高级方面，它从* *Ant* *借用了绝大多数构建任务。*
...
**gradle命令**
**#** **查看tasks, task**表示一个逻辑上较为独立的执行过程；查看项目属性列表
$ gradle tasks
$ gradle properties
\# 查看依赖列表
...
```
bare .sbt 构建定义[ ](http://www.scala-sbt.org/release/docs/zh-cn/Basic-Def.html#bare+.sbt+构建定义)
...
| yum                                  | red-hat    | $ yum install gradle        |
...
| [Python](http://en.wikipedia.org/wiki/Python_(programming_language)) | 法1：直接把单文件拷贝到$python_dir/lib   法2：pip安装   法3：python setup.py install | from   xxx import xxx   import   xxx                         |
| [Ruby](http://en.wikipedia.org/wiki/Ruby_programming_language) | [RubyGems](https://rubygems.org/)安装                        |                                                              |
| [Java](http://en.wikipedia.org/wiki/Java_(programming_language)) | 直接把jar放到lib目录                                         | 使用时packet, import                                         |
| node.js                                                      | **npm**安装                                                  | require                                                      |
| [PHP](http://en.wikipedia.org/wiki/PHP)                      | 静态编译或动态编译，**pear**安装                             |                                                              |
| [Perl](http://en.wikipedia.org/wiki/Perl)                    | 方法一编译安装；   方法二自动安装perl -MCPAN -e shell;   WINDOWS下还可使用PPM图形安装。 |                                                              |
| [Scala](http://en.wikipedia.org/wiki/Scala_programming_language) |                                                              |                                                              |
| [Go](http://en.wikipedia.org/wiki/Go!_(programming_language)) |                                                              |                                                              |
| [Erlang](http://en.wikipedia.org/wiki/Erlang_programming_language) |                                                              |                                                              |
|                                                              |                                                              |                                                              |
...
password: 
```
ppi源配置：windows下~/pip/pip.ini，linux下~/.pip/pip.conf，可以配置多源
```SHLLL
# pip.conf
[global] 
...
[**Pypy Python的JIT实现**](http://www.cnblogs.com/hitfire/articles/4502199.html)
...
[3].     [Pypy Python的JIT实现](http://www.cnblogs.com/hitfire/articles/4502199.html)http://www.cnblogs.com/hitfire/articles/4502199.html 
[4].     JIT编译器 [https://baike.baidu.com/item/JIT%E7%BC%96%E8%AF%91%E5%99%A8/3793585](https://baike.baidu.com/item/JIT编译器/3793585) 
...
The commands are:
    ...
    instal*     compile and install packages and dependencies
    list        list packages
...
[2].     tiobe编程语言排行http://www.tiobe.com/index.php/content/paperinfo/tpci/index.htm*  
