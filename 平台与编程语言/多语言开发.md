...
| C++    | ISO C++98/03关键字共63个。   C++11新增10个。<br>C++17 | C语言中没有的关键字如下：  <br> *  类型转化（5个）：dynamic_cast、static_cast、reinterpret_cast const_cast reinterpret_cast   <br/>*  面向对象00（10个）：this class virtual  public private protected friend namespace template   using   *  内存分配与异常处理（5个）：new delete try   catch throw   <br/>*  其它：wchar_t mutable |
...
| Python | python 2.7共31个。<br>python 3共33个。 | 关键字分类如下：   <br/>*  控制流（16个）：if else while   for break elif and not or with as   pass for in return continue   <br/>*  异常（4个）： try except   raise finally   <br/>*  模块（3个）：from del  import   <br/>*  其它（7个）：global yield def lambda assert print exec |
|go|||
...
> 备注：变量类型为type；变量名为VAL，函数名为FUN，参数为p。以上的函数调用不包括类内的函数。
