...
说明：不同颜色代表一个流行分支。unix主要发展出以下分支：BSD(1978)、Solaris(1992)、 Linux(1991).
...
查看内核版本： `cat /proc/version`  或者 `uname -a`  或者 `lsb_release -a`
...
CentOS Linux发行版是一个稳定的，可预测的，可管理的和可复现的平台，源于[Red Hat Enterprise Linux](https://baike.baidu.com/item/Red Hat Enterprise Linux/10770503)（RHEL）依照开放源代码（大部分是GPL开源协议 [2] ）规定释出的源码所编译而成。
...
| 4.1                             | Memphis                                                      | [Windows  98](https://baike.baidu.com/item/Windows 98)       | 1998-6-25    |
...
1. 发布日期和发行日期：发布日期是首次对外宣传的日期，发行日期是正式上市的时间，发行日期一般会比发布日期更慢。
...
1. **文件系统的逻辑结构**: (用户可见的)包括1)无结构的字符流文件; 2)有结构的记录文件
2. **文件系统的物理结构**(在物理存储空间的存放组织方式):  以文件物理块作为分配的基本单位. 可分为 1)连续文件;2)链接文件;3)索引文件.

3. 文件目录

使用FCB(File Control Block).

文件目录结构:1)b treee; 2)linux的i接点
...
**MBR** (Master Boot Record): 主引导记录, X86上，MBR使用系统分区上的引导扇区来装载操作系统的核心文件. 结构=主引导记录+分区表+0x55AA



EBR: Extend Boot Record, 扩展引导记录, 引导扇区的内容.

**EBR引导扇区的结构: (位于**整个硬盘的0柱面0磁头1扇区,硬盘的第**1扇区,512byte)**
...
- 超级块主要功能是给出文件系统各个部分的大小，其中包含描叙文件系统布局的信息。
...
- 《[linux内核源码剖析](../源码剖析/linux内核源码剖析.md)》
- 《[linux内核同步机制分析](../源码剖析/linux内核同步机制分析.md)》
...
* IBM development Linux 内核专题http://www.ibm.com/developerworks/cn/linux/theme/kernel/?ca=drs-tp1808
* [M.  Tim Jones](http://www.ibm.com/developerworks/cn/linux/l-linux-networking-stack/index.html#author)  Linux 网络栈剖析 http://www.ibm.com/developerworks/cn/linux/l-linux-networking-stack/index.html 2007.7.16
