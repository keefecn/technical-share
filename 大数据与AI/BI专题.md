...
* [FineBI](https://www.fanruan.com/finebi)  新一代自助大数据分析的BI工具
...
* 数据连接：指报表元数据连接。内置数据库是本地数据库H2，用户初学者可以使用这个。外置数据库一般用在正式场合。
...
* 报表预览：
  ...
  * 分页预览：此处分页只是对超过页面设置大小的内容（针对的是数据内容长度）进行自动分页，主要设置项在菜单-页面设置。
...
* 数据准备：可以导入多种数据集。数据分实时和抽取，实时只能使用EXCEL表和自助数据集（相当于上传文件，然后作数据预处理：字段类型转化、值填充），抽取则是常规数据集。
* 自助分析：图表可以复用到其它仪表板。
* 查看分析：
